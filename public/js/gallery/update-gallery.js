		...
		$('#gallery-create-form').submit(function (e) {
			...
			$.ajax({
				...
				beforeSend:function () {
					...
					$('.gallery-edit-wrap').css({ opacity: 0.5 });
					$('.gallery-edit-wrap').parent().prepend(loader);
				},
				success: function(data){
					window.location.replace('/gallery/'+data.id+'/'+data.title);
				},
				error: function (data) {
					$('#submit-btn').attr('disabled', false);
					$('.gallery-edit-wrap').css({ opacity: 1 });
					$("#loader").remove();
					let once = true;
